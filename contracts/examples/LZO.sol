...
contract LZO is UniversalONFT721 {
    ...
    function generateSVG(uint256 tokenId) public view returns(string memory) {
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="50%" y="40%" class="base" dominant-baseline="middle" text-anchor="middle">', "ChainID: ", Strings.toString(lzEndpoint.getChainId()), '</text>',
            '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle">', "TokenID: ", Strings.toString(tokenId), '</text>',
            '</svg>'
        );
        return string(
            abi.encodePacked(
